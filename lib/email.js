...
/** Email a 6-digit verification code (the signup OTP flow). */
export async function sendVerificationCode(to, code) {
  return getResend().emails.send({
    from: FROM,
    to,
    subject: `${code} is your PulseFy verification code`,
    html: shell(
      "Verify your email",
      `<p>Welcome to PulseFy! Enter this code to activate your account:</p>
       <p style="text-align:center;margin:24px 0;">
         <span style="display:inline-block;font-family:'SF Mono',Menlo,Consolas,monospace;font-size:34px;font-weight:800;letter-spacing:10px;color:#f3f5f7;background:#12141a;border:1px solid #262a35;border-radius:12px;padding:16px 20px 16px 30px;">${esc(code)}</span>
       </p>
       <p style="font-size:13px;color:#6b7280;">This code expires in 10 minutes. If you didn't sign up, you can ignore this email.</p>`
    ),
  });
}
...
/**
 * Tell a creator their submission was approved. Best-effort: called from the
 * review route after the reward transaction has already committed, so a send
 * failure here must never surface to the reviewer.
 */
export async function sendApprovalEmail(to, { name, challenge, reward, spotlightBonus, dashboardUrl }) {
  const rewardNum = Number(reward) || 0;
  const bonusNum = Number(spotlightBonus) || 0;
  const earned = rewardNum + bonusNum;
  const greeting = name ? `Hi ${esc(name)},` : "Hi,";
  const rewardBlock =
    earned > 0
      ? `<p style="text-align:center;margin:24px 0;">
           <span style="display:block;font-size:12px;color:#6b7280;letter-spacing:.08em;text-transform:uppercase;margin-bottom:10px;">You earned</span>
           <span style="display:inline-block;font-family:'SF Mono',Menlo,Consolas,monospace;font-size:34px;font-weight:800;color:#ffb43a;background:#12141a;border:1px solid #262a35;border-radius:12px;padding:14px 28px;">$${esc(earned)}</span>
           ${bonusNum > 0 ? `<span style="display:block;font-size:13px;color:#6b7280;margin-top:12px;">includes a $${esc(bonusNum)} Spotlight bonus</span>` : ""}
         </p>`
      : "";
  return getResend().emails.send({
    from: FROM,
    to,
    subject: "Your PulseFy submission was approved",
    html: shell(
      "Your submission was approved",
      `<p>${greeting}</p>
       <p>Great news — your submission${challenge ? ` to <b>${esc(challenge)}</b>` : ""} has been approved.</p>
       ${rewardBlock}
       <p>${button(dashboardUrl, "View your dashboard →")}</p>
       <p style="font-size:13px;color:#6b7280;">Your earnings and submission status are always available on your PulseFy dashboard.</p>`
    ),
  });
}

/**
 * Tell an Ambassador applicant their application was approved. Best-effort,
 * called from the ambassador review route. The applicant's email lives on the
 * application row, so this reaches them whether or not they have an account.
 */
export async function sendAmbassadorApprovalEmail(to, { name, url }) {
  const greeting = name ? `Hi ${esc(name)},` : "Hi,";
  return getResend().emails.send({
    from: FROM,
    to,
    subject: "You're in — welcome to the PulseFy Ambassador Program",
    html: shell(
      "You're a PulseFy Ambassador",
      `<p>${greeting}</p>
       <p>Great news — your application to the <b>PulseFy Ambassador Program</b> has been approved. Welcome aboard!</p>
       <p>Our team will follow up personally with your next steps.</p>
       ${url ? `<p>${button(url, "View the Ambassador Program →")}</p>` : ""}`
    ),
  });
}
