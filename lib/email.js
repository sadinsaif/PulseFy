...
const FROM = process.env.EMAIL_FROM || "Srijon <onboarding@resend.dev>";
    ...
    <div style="max-width:480px;margin:0 auto;background:#16181f;border:1px solid #262a35;border-radius:16px;overflow:hidden;">
      <div style="padding:28px 32px;border-bottom:1px solid #1e2129;">
        <span style="display:inline-flex;align-items:center;gap:10px;color:#f3f5f7;font-weight:800;font-size:20px;">
          <span style="width:28px;height:28px;border-radius:8px;background:linear-gradient(135deg,#ff7a45,#ffb43a);display:inline-block;text-align:center;line-height:28px;color:#0a0b0d;font-weight:900;">S</span>
          Srijon
        </span>
      </div>
      ...
      <div style="padding:20px 32px;border-top:1px solid #1e2129;color:#6b7280;font-size:12px;">
        © 2026 Srijon · You received this because someone used this email on Srijon.
      </div>
    </div>
  </div>`;
}
...
export async function sendVerificationEmail(to, url) {
  return resend.emails.send({
    from: FROM,
    to,
    subject: "Verify your Srijon account",
    html: shell(
      "Verify your email",
      `<p>Welcome to Srijon! Confirm your email address to activate your account.</p>
...
export async function sendResetEmail(to, url) {
  return resend.emails.send({
    from: FROM,
    to,
    subject: "Reset your Srijon password",
    html: shell(
      "Reset your password",
