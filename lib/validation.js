...
// A $PULSE reward claim. `amount` is a decimal token string (e.g. "12.5"), not
// base units — the route converts it with parseTokens() (which rejects more
// fractional digits than the mint's decimals) and validates it against the
// holder's DERIVED available balance. There is no `destination` field on
// purpose: a claim always pays the user's own VERIFIED linked wallet (server-
// authoritative), so no one can claim rewards out to an address they don't own.
...
// A submission review by an admin/moderator: approve/reject with verified
// metrics, an optional reward override, and an optional spotlight bonus.
